...
impl App for NonKeyed {
    ...
    fn __render(&mut self, mb: &Mailbox<Self>) {
        if self.t_root & 0b0000_0001 != 0 {
            ...
            if dom_len < row_len {
                ...
                for row in self.data[dom_len..].iter() {
                    ...
                    self.tbody.append_child(&v.root).unwrap();
                ...
                for _ in 0..dom_len - row_len {
                    self.tbody
                        .remove_child(&self.tbody.last_child().unwrap())
                        .unwrap();
                }
        ...
        if self.t_root & 0b0000_0010 != 0 {
            ...
            if let Some(old) = self.old_selected.take() {
                children.item(old as u32).unwrap().set_class_name("")
            }
            if let Some(new) = self.selected {
                if let Some(new) = self.data.iter().position(|x| x.id == new) {
                    children.item(new as u32).unwrap().set_class_name("danger");
    ...
    fn __hydrate(&mut self, mb: &Mailbox<Self>) {
        ...
        let f = document
            ...
            .unwrap()
            .first_child()
            .unwrap(); // div.jumbotron

        let f_0 = f.first_child().unwrap(); // div.row
        let f_0_0 = f_0.first_child().unwrap(); // div.col-md-6
        let f_0_1 = f_0_0.next_sibling().unwrap(); // div.col-md-6
        let f_0_1_0 = f_0_1.first_child().unwrap(); // div.row

        let f_0_1_0_0 = f_0_1_0.first_child().unwrap(); // div.col-sm-6 smallpad
        let button_create = f_0_1_0_0.first_child().unwrap(); // button CREATE 1_000
        let cloned = mb.clone();
        ...
        let f_0_1_0_1 = f_0_1_0_0.next_sibling().unwrap(); // div.col-sm-6 smallpad
        let button_create_10 = f_0_1_0_1.first_child().unwrap(); // button CREATE 10_000
        let cloned = mb.clone();
        let cl = Closure::wrap(Box::new(move |event: Event| {
            event.prevent_default();
        ...
        let f_0_1_0_2 = f_0_1_0_1.next_sibling().unwrap(); // div.col-sm-6 smallpad
        let button_append = f_0_1_0_2.first_child().unwrap(); // button  APPEND 1_000
        let cloned = mb.clone();
        let cl = Closure::wrap(Box::new(move |event: Event| {
            event.prevent_default();
            cloned.send(Append)
        ...
        let f_0_1_0_3 = f_0_1_0_2.next_sibling().unwrap(); // div.col-sm-6 smallpad
        let button_update = f_0_1_0_3.first_child().unwrap(); // button  UPDATE
        let cloned = mb.clone();
        let cl = Closure::wrap(Box::new(move |event: Event| {
            event.prevent_default();
            cloned.send(Update);
        ...
        let f_0_1_0_4 = f_0_1_0_3.next_sibling().unwrap(); // div.col-sm-6 smallpad
        let button_clear = f_0_1_0_4.first_child().unwrap(); // button CLEAR
        let cloned = mb.clone();
        let cl = Closure::wrap(Box::new(move |event: Event| {
            event.prevent_default();
            cloned.send(Clear);
        ...
        let f_0_1_0_5 = f_0_1_0_4.next_sibling().unwrap(); // div.col-sm-6 smallpad
        let button_swap = f_0_1_0_5.first_child().unwrap(); // button SWAP
        let cloned = mb.clone();
        let cl = Closure::wrap(Box::new(move |event: Event| {
            event.prevent_default();
            cloned.send(Swap)
...
impl Default for NonKeyed {
    fn default() -> Self {
        ...
        let f = node.first_child().unwrap(); // div.jumbotron
        let n1 = f.next_sibling().unwrap(); // table.table table-hover table-striped test-data
        let tbody = n1.first_child().unwrap(); // tbody

        let mut tbody_children = vec![];
        if let Some(mut curr) = tbody.first_child() {
            loop {
                let id_node = curr.first_child().unwrap();
                let label_parent = id_node.next_sibling().unwrap();
                let label_node = label_parent.first_child().unwrap();
                let delete_parent = label_parent.next_sibling().unwrap();
                let delete_node = delete_parent.first_child().unwrap();
