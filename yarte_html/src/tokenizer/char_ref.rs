...
use markup5ever::{buffer_queue::BufferQueue, data};

use crate::utils::is_ascii_alnum;
...
impl CharRefTokenizer {
    ...
    fn finish_numeric<Sink: TokenSink>(&mut self, tokenizer: &mut Tokenizer<Sink>) -> Status {
        ...
        let (c, error) = match self.num {
            n if (n > 0x0010_FFFF) || self.num_too_big => ('\u{fffd}', true),
    ...
    fn emit_name_error<Sink: TokenSink>(&mut self, tokenizer: &mut Tokenizer<Sink>) {
        let msg = Owned(format!("Invalid character reference &{}", self.name_buf()));
        tokenizer.emit_error(msg);
    }
