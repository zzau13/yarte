...
use markup5ever::{
    buffer_queue::{BufferQueue, FromSet, NotFromSet, SetResult},
    namespace_url, ns, small_char_set,
    tendril::StrTendril,
    LocalName, SmallCharSet,
};
...
pub use self::interface::{
    CharacterTokens, CommentToken, Doctype, DoctypeToken, EOFToken, EndTag, NullCharacterToken,
    ParseError, StartTag, Tag, TagKind, TagToken, Token, TokenSink, TokenSinkResult,
};

use self::states::{
    DoctypeIdKind, DoubleEscaped, DoubleQuoted, Escaped, Public, Rawtext, Rcdata, ScriptData,
    ScriptDataEscaped, SingleQuoted, System, Unquoted,
};
...
pub struct Tokenizer<Sink> {
    ...
    last_start_tag_name: Option<YName>,
...
impl<Sink: TokenSink> Tokenizer<Sink> {
    ...
    fn bad_eof_error(&mut self) {
        let msg = Owned(format!("Saw EOF in state {:?}", self.state));
        self.emit_error(msg);
    }
...
mod test {
    ...
    use super::option_push;
    // private items

    use super::{TokenSink, TokenSinkResult, Tokenizer};

    use super::interface::{
        CharacterTokens, EOFToken, EndTag, NullCharacterToken, ParseError, StartTag, Tag, TagKind,
        TagToken, Token,
    };

    use crate::interface::YName;
    use markup5ever::{
        buffer_queue::BufferQueue,
        tendril::{SliceExt, StrTendril},
        LocalName,
    };
    ...
    fn tokenize(input: Vec<StrTendril>) -> Vec<(Token, u64)> {
    ...
    fn create_tag(token: StrTendril, tagkind: TagKind) -> Token {
        let name = YName::Local(LocalName::from(&*token));
        TagToken(Tag {
            kind: tagkind,
            name,
            self_closing: false,
